...
module.exports ={
    ...
    Rate : async function(namespace, route, size, criteria, headers=null, username){
        ...
        return await queueMan(route, rateObj);
    ...
    validate : async function (namespace, type, size, criteria, username ){
        ...
        let isValid = await this.validateWithRedis(key,size);
        if (!isValid){
...
function createRules(namespace, userid, obj, id, expiry) {
    return new Promise(function(resolve, reject) {
        ...
        obStore.insertSingle("ratingenginerules", namespace, "id", ruleObj, (response)=>{
            if(response.IsSuccess === true){
                let result = sendRuleToQueue(ruleObj);
                if(result === "DONE"){
...
async function  sendRuleToQueue(rules) {
    ...
    rateObj.route = "rule";
    let result = await queueMan("rule", rateObj);
    if(result === "DONE"){
        return(result);
    }
    else{
